...
def parse_arguments() :
    ...
    parser = argparse.ArgumentParser(description='Horseman No. 1 - an Australian epidemic simulator')
    parser.add_argument( '-t', '--test', default = False, action = 'store_true', help = 'Activates TESTING mode (default: deactivated)')
    parser.add_argument( '-s', '--scenario', default = None, type = str, help = 'Path to scenario (.scn) file')
    parser.add_argument( '-a', '--agent', default = 'HealthAgent', type = str, help = 'Agent to be used (default: HealthAgent)')
    parser.add_argument( '-H', '--horizon', default = 100, type = int, help = 'Maximum number of steps in the simulation (default: 100)')
    parser.add_argument( '-n', '--num_sims', default = 1, type = int, help = 'Number of simulations to run (default: 1)' )
    ...
    if args.test :
        ...
        try:
            ...
            if not ds.read_scenario_file(args.scenario):
                print("Error: the supplied scenario file does not exist:", args.scenario )
                sys.exit()
        except Exception:
            print("Unexpected error while loading the specified disease scenario:", args.scenario)
            print(traceback.format_exc())
        ...
        try:
            ...
            for j in range( 0, args.num_sims ) :
                ...
                for i in range(iterations):
                    ...
                    if total_disease >= 100000:
                        ...
                        print("Everybody is dead, kangaroos, wallabies and wombats roam freely across the land.")
                        break
                sim_data = {}
                ...
                if total_disease < 100000 :
                    if i < args.horizon - 1 :
                        ...
                        sim_data['score'] = 1.0 - float(total_disease)/float(100000)
            ...
            with open( '{}.results.json'.format(os.path.basename(args.scenario).replace('.scn','')), 'w' ) as output :
                output.write(json.dumps(simulations_data, sort_keys=True, indent=4))

...
from planning_task import Task, Action
...
def ground(problem):
    ...
    Problem.domain.actions      -> actions    -> Task.actions
    Problem.domain.predicates   -> variables    -> Task.facts
    """
    domain = problem.domain
    actions_schema = domain.actions.values()
    ...
    statics = _get_statics(predicates, actions_schema)
    if verbose_logging:
    ...
    # Ground actions
    actions = _ground_actions(actions_schema, type_map, statics, init)
    if verbose_logging:
    ...
    # Collect facts from actions and include the ones from the goal
    facts = _collect_facts(actions) | goals
    if verbose_logging:
    ...
    actions = _relevance_analysis(actions, goals)

    name = problem.name
    return Task(name, facts, init, goals, actions)


def _relevance_analysis(actions, goals):
    """This implements a relevance analysis of actions.
    ...
    while changed:
        ...
        # compute cut of relevant facts with effects of all actions
        for op in actions:
    ...
    if debug:
        logging.info('Relevance analysis removed %d facts' %
                     len(debug_pruned_op))
    # remove completely irrelevant actions
    return [op for op in actions if not op in del_actions]


def _get_statics(predicates, actions_schema):
    """
    ...
    effects = [get_effects(action) for action in actions_schema]
...
def _collect_facts(actions):
    """
    Collect all facts from grounded actions (precondition, add
...
def _ground_actions(actions_schema, type_map, statics, init):
    """
    Ground a list of actions_schema and return the resulting list of actions.

    @param actions_schema: List of actions_schema
    ...
    actions = list(itertools.chain(*op_lists))
    return actions
...
def _ground_action(action, type_map, statics, init):
    """
    Ground the action and return the resulting list of actions.
    """
    ...
    if verbose_logging:
        logging.info('Static precondition analysis removed %d possible objects'
                     % remove_debug)
    logging.debug('Static precondition analysis removed %d possible objects' % remove_debug)
    ...
    # Create a new action for each possible assignment of parameters
    ops = [_create_action(action, dict(assign), statics, init)
...
def _create_action(action, assignment, statics, init):
    """Create an action for "action" and "assignment".

    Statics are handled here. True statics aren't added to the
    precondition facts of a grounded action. If there is a false static
    in the ungrounded precondition, the action won't be created.
    ...
    for precondition in action.precondition:
        ...
        if predicate_name in statics:
            ...
            if fact not in init:
                # This precondition is never true -> Don't add action
                return None
        else:
    ...
    # If the same fact is added and deleted by an action the STRIPS formalism
    # adds it.
    ...
    return Action(name, precondition_facts, add_effects, del_effects)

...
class _RelaxationHeuristic(Heuristic):
	...
	def reachability(self, queue):
		...
		while not self.finished(achieved_goals, queue):
			...
			if not fact.expanded:  #对一个fact 进行操作
				# Iterate over all actions this fact is a precondition of.
				for action in fact.precondition_of:   # 对以这个fact为precondition的action进行操作
...
class hadm(_RelaxationHeuristic):
	...
	def __call__(self, node):
		...
		count = 0
		for goal in self.goals:
			if self.facts[goal].level == 0:
				count = count + 1
		return (total_goals - count)
		
		if count > math.floor(total_goals/2):
			return 0
		else:
			return 1
